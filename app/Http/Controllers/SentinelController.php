...
class SentinelController extends Controller
{
    ...
    public function authenticate($credentials, $remember = false, $login = true)
    {
        ...
        if ($credentials instanceof UserInterface) {
            ...
            if (count($this->users->findByCredentials1($credentials)) > 1) {
                $a=array();
                foreach($this->users->findByCredentials1($credentials) as $usuario){
                    //                for($i=0;$i<count($this->users->findByCredentials1($credentials));$i++) {
//                   return  $this->users->findByCredentials($credentials);
                    $user = $this->users->findByCredentials($credentials);
                    $valid = $user !== null ? $this->users->validateCredentials($usuario,$credentials) : false;
//                            return $user2['id'];
                    array_push($a,$valid);
...
//                if (! $this->cycleCheckpoints('login', $user2)) {
//                    return false;
//                }
//
//                if ($login === true) {
//                    $method = $remember === true ? 'loginAndRemember' : 'login';
//
//                    if (! $user2 = $this->{$method}($user2)) {
//                        return false;
//                    }
//                }
//                $this->fireEvent('sentinel.authenticated', $user2);
